...
import { ActivityIndicator, DeviceEventEmitter, Image, KeyboardAvoidingView, Platform, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Ionicons } from '@expo/vector-icons';
import { StatusBar } from 'expo-status-bar';
import { useLocalSearchParams, useRouter } from 'expo-router';
import Animated, { Easing, FadeInRight, runOnJS, useAnimatedStyle, useSharedValue, withTiming } from 'react-native-reanimated';
...
export default function AuthScreen() {
  ...
  useEffect(() => { panelProgress.value = withTiming(1, { duration: 480, easing: Easing.out(Easing.cubic) }); }, [panelProgress]);
  const panelEntrance = useAnimatedStyle(() => ({ opacity: panelProgress.value, transform: [{ translateY: (1 - panelProgress.value) * 620 }] }));
  const finishClose = useCallback(() => router.back(), [router]);
  const closeModal = useCallback(() => {
    if (closing) return;
    setClosing(true);
    DeviceEventEmitter.emit('lumio-auth-closing');
    panelProgress.value = withTiming(0, { duration: 420, easing: Easing.inOut(Easing.cubic) }, (finished) => {
      if (finished) runOnJS(finishClose)();
    });
  }, [closing, finishClose, panelProgress]);
  return <SafeAreaView style={s.safe} edges={['top', 'bottom']}><StatusBar style="dark" /><KeyboardAvoidingView style={s.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}><View style={s.page}>
    <Animated.View style={[s.panel, panelEntrance]}><TouchableOpacity style={s.back} onPress={closeModal} activeOpacity={.7} disabled={closing}><Ionicons name="arrow-down" size={19} color={Colors.primary} /></TouchableOpacity>
      ...
      <ScrollView style={s.formScroll} contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><Animated.View key={active} entering={FadeInRight.duration(220)}>{active === 'login' ? <Login done={(v) => router.replace(v ? '/(tabs)/chat' : '/onboarding')} /> : <Register done={() => router.replace('/onboarding')} />}</Animated.View></ScrollView>
    </Animated.View>
...
const s = StyleSheet.create({
  safe: { flex: 1, backgroundColor: 'transparent' }, flex: { flex: 1 }, page: { flex: 1, overflow: 'hidden', backgroundColor: 'transparent' },
