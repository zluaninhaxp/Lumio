...
export default function ProfileScreen() {
  ...
  const pickPhoto = async () => { const permission = Platform.OS === 'web' ? { granted: true } : await ImagePicker.requestMediaLibraryPermissionsAsync(); if (!permission.granted) return Alert.alert('Permissão necessária', 'Permita o acesso às fotos para escolher uma imagem.'); const result = await ImagePicker.launchImageLibraryAsync({ mediaTypes: ['images'], allowsEditing: true, aspect: [1, 1], quality: 0.8 }); if (!result.canceled) setPhoto(result.assets[0].uri); };
  const choosePhoto = () => Platform.OS === 'web' ? pickPhoto() : Alert.alert('Foto do perfil', undefined, [{ text: 'Cancelar', style: 'cancel' }, { text: 'Escolher da galeria', onPress: pickPhoto }, ...(photo ? [{ text: 'Remover foto', style: 'destructive' as const, onPress: () => setPhoto(null) }] : [])]);
