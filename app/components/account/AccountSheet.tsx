...
export function AccountSheet({ visible, onClose }: { visible: boolean; onClose: () => void }) {
  ...
  const confirmLogout = () => Alert.alert('Sair da conta', 'Deseja realmente sair?', [
    ...
    { text: 'Sair', style: 'destructive', onPress: async () => { try { await logout(); onClose(); router.replace('/welcome'); } catch { Alert.alert('Erro', 'Não foi possível sair. Tente novamente.'); } } },
  ]);
