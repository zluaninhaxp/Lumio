...
import { TaskPeopleSelector } from '../Tasks/TaskPeopleSelector';
...
export function QuickAddForm({ onSave, onCancel, editData, categories, incomeCategories }: QuickAddFormProps) {
  ...
  const { fornecedorItems, estoqueItems, activatedPlugins, receiveStockFromPurchase, addFinancialExpenseCategory, addFinancialIncomeCategory } = useAppStore();
        ...
        <>
          <TaskPeopleSelector
            ...
            onChange={(relation, id) => {
              ...
              else if (relation === 'supplier') {
                setSupplierId(id);
                if (!id) setSupplierDueDate('');
                else {
                  const supplier = fornecedorItems.find((candidate) => candidate.id === id);
