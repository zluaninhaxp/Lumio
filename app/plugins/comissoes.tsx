  ...
  const handlePay = (employeeId: string, name: string, amount: number) =>
    Alert.alert(
      ...
      [
        ...
        {
          text: "Concluir",
          onPress: () => closeEmployeeCommission(employeeId),
        },
      ],
    );
    ...
    Alert.alert(
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          ...
          style: "destructive",
          onPress: () => {
            setPluginActivation("comissoes", false);
            router.back();
          },
        },
      ],
      ...
      >
        <View style={styles.summaryCard}>
          ...
          <Text style={styles.summaryHint}>
            Comissões devidas por pedidos concluídos. Conclua o saldo quando
            fizer o fechamento.
          </Text>
        </View>
          ...
          <View key={entry.employeeId} style={styles.card}>
            <View style={styles.avatar}>
              <Text style={styles.avatarText}>
                {entry.employee!.name.slice(0, 1).toUpperCase()}
              </Text>
            </View>
            <View style={styles.cardMain}>
              <Text style={styles.cardTitle}>{entry.employee!.name}</Text>
              <Text style={styles.cardSubtitle}>
                {entry.employee!.role} · {entry.count} pedido(s)
              </Text>
            </View>
                ...
                <View key={c.id} style={styles.historyRow}>
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={styles.historySubtitle}>
                      {formatMonth(c.month)} · {c.rate}% · pedido{" "}
                      {c.orderId.slice(-6)}
                    </Text>
                  </View>
