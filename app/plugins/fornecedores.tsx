...
export default function FornecedoresScreen() {
  ...
  const {
    ...
    transactions,
    addFornecedorItem,
    updateFornecedorItem,
    removeFornecedorItem,
    linkTransactionToSupplier,
    setPluginActivation,
  } = useAppStore();
  ...
  const [unlinkedTransactionId, setUnlinkedTransactionId] = useState<
    string | null
  >(null);

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    return fornecedorItems.filter(
      (item) => !q || `${item.name} ${item.contact}`.toLowerCase().includes(q),
    );
  }, [fornecedorItems, query]);
  const unlinkedExpenses = transactions.filter(
    (transaction) =>
      transaction.amount < 0 &&
      transaction.category === "Fornecedores" &&
  ...
  const openEdit = (item: FornecedorItem) => {
    ...
    Alert.alert(
      ...
      [
        { text: "Cancelar", style: "cancel" },
        {
          text: "Excluir",
          style: "destructive",
          onPress: () => removeFornecedorItem(id),
        },
      ],
    );
  const linkOldExpense = (supplier: FornecedorItem) => {
    if (!unlinkedTransactionId) return;
    const transaction = transactions.find(
      (item) => item.id === unlinkedTransactionId,
    );
    if (!transaction) return;
    linkTransactionToSupplier(transaction.id, supplier.id, {
      supplierDueDate: suggestedDueDate(transaction.date, supplier.paymentTerm),
      supplierPaid: false,
    });
    setUnlinkedTransactionId(null);
  };
            ...
            Alert.alert(
              ...
              [
                { text: "Cancelar", style: "cancel" },
                {
                  ...
                  style: "destructive",
                  onPress: () => {
                    setPluginActivation("fornecedores", false);
                    router.back();
                  },
                },
              ],
      ...
      >
        ...
        {filtered.map((supplier) => {
          const history = transactions.filter(
            (transaction) =>
              transaction.supplierId === supplier.id && transaction.amount < 0,
          );
          const debt = history
            .filter((transaction) => !transaction.supplierPaid)
            .reduce(
            ...
            <View key={supplier.id} style={styles.card}>
              ...
              >
                ...
                <View style={styles.cardMain}>
                  ...
                  <Text style={styles.cardSubtitle}>
                    {supplier.contact || "Sem contato informado"} ·{" "}
                    {supplier.paymentTerm || "Prazo não informado"}
                  </Text>
                </View>
                <Ionicons
              ...
              <View style={styles.summaryRow}>
                <Text style={styles.summaryLabel}>
                  {history.length} compra(s) vinculada(s)
                </Text>
                <Text style={[styles.total, debt === 0 && styles.paidTotal]}>
                  {debt > 0 ? `Deve ${money(debt)}` : "Sem saldo devedor"}
                </Text>
              </View>
              {expanded && (
                <View style={styles.details}>
                  ...
                  {history.length === 0 ? (
                    <Text style={styles.muted}>Nenhuma despesa vinculada.</Text>
                  ) : (
        ...
        {unlinkedExpenses.length > 0 && (
          <View style={styles.migration}>
            <Text style={styles.migrationTitle}>Despesas sem fornecedor</Text>
            <Text style={styles.muted}>
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            />
            <FormLabel>Prazo padrão (opcional)</FormLabel>
            <TextInput
              style={styles.input}
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={styles.linkCard}>
            <Text style={styles.modalTitle}>Associar fornecedor</Text>
            <Text style={styles.muted}>
...
const styles = StyleSheet.create({
  ...
  notes: { color: Colors.textSecondary, marginBottom: Spacing.md },
  historyTitle: {
    fontFamily: "PlusJakartaSans_600SemiBold",
    color: Colors.primary,
    marginBottom: Spacing.sm,
