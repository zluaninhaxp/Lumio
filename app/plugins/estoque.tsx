...
const EMPTY_FORM = { name: '', quantity: '', unit: '', category: '', minAlert: '' };

export default function EstoqueScreen() {
  const router = useRouter();
  const { estoqueItems, addEstoqueItem, updateEstoqueItem, removeEstoqueItem, setPluginActivation } =
    useAppStore();
  ...
  const handleSave = () => {
    ...
    const payload = {
      ...
      minAlert: Number(form.minAlert) || 0,
    };
    if (editingId) {
      updateEstoqueItem(editingId, payload);
    } else {
    ...
    Alert.alert(
      ...
      [
        { text: 'Cancelar', style: 'cancel' },
        {
          ...
          style: 'destructive',
          onPress: () => {
            setPluginActivation('estoque', false);
            router.back();
          },
        },
      ]
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        {estoqueItems.length === 0 && (
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Nenhum item no estoque ainda.</Text>
          </View>
        )}
          ...
          >
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.cardSubtitle}>
                {item.category || 'Sem categoria'} · {item.quantity} {item.unit}
              </Text>
            </View>
            {item.minAlert > 0 && item.quantity <= item.minAlert && (
              <View style={styles.alertBadge}>
                <Ionicons name="alert-circle" size={14} color={Colors.danger} />
                <Text style={styles.alertBadgeText}>Baixo</Text>
              </View>
            )}
          </TouchableOpacity>
        ))}
      ...
      <Modal visible={modalVisible} transparent animationType="slide">
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <TextInput
              style={styles.input}
              placeholder="Alerta de mínimo"
              placeholderTextColor={Colors.textMuted}
              value={form.minAlert}
              onChangeText={(v) => setForm((f) => ({ ...f, minAlert: v }))}
              keyboardType="numeric"
            />
...
const styles = StyleSheet.create({
  ...
  alertBadgeText: {
    fontFamily: 'PlusJakartaSans_600SemiBold', fontSize: FontSize.xs, color: Colors.danger,
  },
  fab: {
